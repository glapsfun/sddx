...
function createRootTask(cwd, spec, specSrc, mode) {
...
function cmdGraphCreate(cwd, args) {
...
function cmdTaskPhase(cwd, args) {
  const [id, phase] = args;
  if (!id || !phase)
    fail(USAGE, 2);
...
function cmdVerify(cwd, args) {
  ...
  if (res.verdict === "pass") {
    console.log(`verdict=pass receipt=${res.receiptPath} commit=${res.commitSha} duration_ms=${res.durationMs}`);
    return;
  }
