...
.sddx/
  ...
  goals/<goal-id>.json       # task ids, run branch, base SHA, merges log, approval provenance;
                             # read by `pr create`. plain state, deliberately never committed
                             # (like sweep.json below)
