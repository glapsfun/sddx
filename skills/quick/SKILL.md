...
- After DONE, report the receipt and commit. The handoff menu is goal-scoped
  (`... next-actions --goal <goal-id>`) and is shown once after a run summary —
  there is no per-task menu. Cleanup: `... cleanup <id>` (deletes only merged
  branches).
